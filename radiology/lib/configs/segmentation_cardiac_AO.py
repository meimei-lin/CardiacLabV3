...
class SegmentationCardiacAO(TaskConfig):
    def __init__(self):
        super().__init__()
    ...
    def init(self, name: str, model_dir: str, conf: Dict[str, str], planner: Any, **kwargs):
        ...
        # Network name
        self.network_name = self.conf.get("network", "unetcnx_x0, unetcnx_a1, testnet, unetwic")
    ...
    def infer(self) -> Union[InferTask, Dict[str, InferTask]]:
        task: InferTask = lib.infers.SegmentationCardiacAO(
            path=self.path,
    ...
    def scoring_method(self) -> Union[None, ScoringMethod, Dict[str, ScoringMethod]]:
        ...
        if self.epistemic_enabled:
            methods[f"{self.name}_epistemic"] = EpistemicScoring(
                model=self.path,
                network=self.network,
                transforms=lib.infers.SegmentationCardiacAO(None).pre_transforms(),
