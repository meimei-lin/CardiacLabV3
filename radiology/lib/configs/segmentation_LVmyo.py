...
class SegmentationLVmyo(TaskConfig):
    def __init__(self):
        super().__init__()
    ...
    def init(self, name: str, model_dir: str, conf: Dict[str, str], planner: Any, **kwargs):
        ...
        # Network name
        self.network_name = self.conf.get("network", "testnet, unetcnx_a1")

        # Model Files
        self.path = [
            os.path.join(self.model_dir, self.network_name, f"best_model_lvmyo.pth"),  # pretrained
            os.path.join(self.model_dir, self.network_name, f"out_model_lvmyo.pth"),  # published
        ]
        ...
        # Transform config
        self.target_spacing = str2list(self.conf.get('target_spacing', '1.0, 1.0, 1.0'))  # target space for image
        self.spatial_size = str2list(self.conf.get('spatial_size', '96, 96, 96')) # train input size
        self.intensity = str2list(self.conf.get('intensity', '-70, 677'))
